from utils.parser import ElementType, MarkdownElement, MarkdownParser, Table, TableCell
...
class ColorConfig:
    def __init__(self, colored_output=True):
        if colored_output:
            ...
            h5_color = (random.randint(160, 255), random.randint(160, 255), random.randint(160, 255))
            ...
            self.H5_COLOR = f"\033[38;2;{h5_color[0]};{h5_color[1]};{h5_color[2]}m"
            ...
            self.HEADING5_TEXT = self.BOLD + self.H5_COLOR
            self.HEADING5_DECORATION = self.H5_COLOR
            ...
            self.H1_COLOR = self.H2_COLOR = self.H3_COLOR = self.H4_COLOR = self.H5_COLOR = ""
            ...
            self.HEADING5_TEXT = self.HEADING5_DECORATION = ""
...
class BoxDrawing:
    ...
    def h5_decoration(self, text: str) -> str:
        c = self.colors
        text_width = len(text)
        line = "=" * text_width
        center_offset = max(0, (self.terminal_width - text_width) // 2)
        padding_str = " " * center_offset
        
        return f"{padding_str}{c.HEADING5_TEXT}{text}{c.RESET}\n{padding_str}{c.HEADING5_DECORATION}{line}{c.RESET}"
...
class TermImageRenderer:
    ...
    def resolve_path(self, img_path):
        ...
        if img_path.startswith("../") or img_path.startswith("..\\"):
            # Extract non-relative parts
            path_parts = img_path.replace("\\", "/").split("/")
            non_dot_parts = [part for part in path_parts if part != ".."]
            
            # Multiple "../" likely targets home directory
            dot_count = path_parts.count("..")
            if dot_count >= 2:
                home = os.path.expanduser("~")
                return os.path.join(home, *non_dot_parts)
        
        # Try source file directory
        if self.source_file_path:
            source_dir = os.path.dirname(os.path.abspath(self.source_file_path))
            resolved_path = os.path.normpath(os.path.join(source_dir, img_path))
            if os.path.exists(resolved_path):
                return resolved_path
        
        # Try working directory
        cwd_path = os.path.normpath(os.path.join(self.cwd, img_path))
        if os.path.exists(cwd_path):
            return cwd_path
        
        # Last resort: use home directory for "../" patterns
        if "../" in img_path or "..\\" in img_path:
            home = os.path.expanduser("~")
            clean_path = img_path.replace("../", "").replace("..\\", "")
            return os.path.join(home, clean_path)
        
        # Fallback to CWD
        return os.path.normpath(os.path.join(self.cwd, img_path))
    ...
    def can_render_images(self):
        ...
        # If source file path is available but no parent directory references
        if self.source_file_path:
            source_dir = os.path.dirname(os.path.abspath(self.source_file_path))
            return os.path.join(source_dir, img_path)
            
        # Otherwise, use current working directory
        return os.path.join(self.cwd, img_path)
    ...
    def render_image(self, img_path: str, caption: str = None) -> str:
        ...
        # Resolve the path
        resolved_path = self.resolve_path(img_path)
        
        # Check if the file exists
        if not os.path.isfile(resolved_path) or not os.access(resolved_path, os.R_OK):
            return f"[Image Not Found: {img_path}] (Resolved path: {resolved_path})"
        
        try:
            # Create and cache the image
            img = from_file(resolved_path)
            self.cache[resolved_path] = img
            
            # Render the image
            return self._draw_image(img, img_path, caption)
        except Exception as e:
            return f"[Image Error: {str(e)}] (Path: {resolved_path})"
    
    def _draw_image(self, img, img_path, caption=None):
        """Draw the image and format it with caption"""
        try:
...
class EnhancedMarkdownRenderer:
    def __init__(self, colored_output=True, source_file_path=None):
        ...
        self.image_renderer = TermImageRenderer(source_file_path)
        self.source_file_path = source_file_path
    
    def _get_plain_text(self, content):
        """Extract plain text from markdown elements"""
        result = ""
        for item in content:
            if isinstance(item, str):
                result += item
    ...
    def render(self, md_text: str) -> str:
        # Pre-process for equals headings
        lines = md_text.split('\n')
        equals_headings = {}
        skip_lines = set()
        ...
        for i in range(1, len(lines)):
            ...
            if line and set(line) == {'='}:
                if i > 0 and lines[i-1].strip():
                    # Store the heading text and mark both lines to be skipped
                    equals_headings[i-1] = lines[i-1].strip()
                    skip_lines.add(i)  # Skip the === line
        
        # Process the markdown normally, but handle equals headings specially
        elements = self.parser.parse(md_text)
        rendered_text = ""
        
        line_num = 0
        for element in elements:
            # Handle equals headings
            if element.type == ElementType.PARAGRAPH and line_num in equals_headings:
                heading_text = equals_headings[line_num]
                element_content = self._get_plain_text(element.content)
                
                # Only render if content matches
                if heading_text == element_content:
                    rendered_text += f"\n{self.box_tools.h5_decoration(heading_text)}\n\n"
                    line_num += 2  # Skip the === line too
                    continue
            
            # Standard processing for all other elements
            ...
            if hasattr(element, 'content') and isinstance(element.content, str):
                line_num += element.content.count('\n') + 1
            elif element.type == ElementType.PARAGRAPH:
                # For paragraphs, count approximate number of lines
                text = self._get_plain_text(element.content)
                line_num += text.count('\n') + 1
            else:
                line_num += 1
...
def render_markdown(md_text: str, colored_output: bool = True, centered: bool = False, source_file_path: str = None) -> str:
    import sys
    
    # Generate the rendered markdown content
    renderer = EnhancedMarkdownRenderer(colored_output=colored_output, source_file_path=source_file_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            with open(file_path, "r") as f:
                md_text = f.read()
            print(f"Rendering markdown from: {file_path}")
    ...
    print(render_markdown(md_text, source_file_path=file_path))
